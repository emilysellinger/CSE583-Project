...
class TestPhoenix(unittest.TestCase):
    ...
    def smoke_test_app_functions(self):
        ...
        """
        data = {'observation date': ['08-01-2020', '09-01-2020','10-01-2020','11-01-2020'],
                'species': ['American Crow', 'American Crow', 'American Crow', 'American Crow']}
        bird_data = pd.DataFrame (data, columns = ['observation date', 'species'])
        month = 'August'
        day = 1

        self.assertTrue(
            isinstance(app_functions.subset_date(bird_data, 'observation date', month, day),
            	pd.DataFrame))
    ...
    def one_shot_test_app_functions(self):
        ...
        data_result = pd.DataFrame(data_result, columns = ['observation date', 'species'])

        self.assertEqual(
            app_functions.subset_date(bird_data, 'observation date', month, day), data_result)
    ...
    def edgetest_app_functions(self):
        ...
        with self.assertRaises(ValueError):
            app_functions.subset_date(bird_data, 'observation date', month, day)
